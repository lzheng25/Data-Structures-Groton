...
public class Extension1 extends WindowController  implements KeyListener{
	...
	boolean flag = false;
    //Initialize variable for the text of incorrectGuesses
	protected String incorrectText = "";
	//Initialize # of incorrect letters to 0
	protected int incorrectCount = 0;
	
	// GUI Elements
	protected Text label;
	protected Text buttonText;
	//Make field for Text for wrong guesses (for label and letters)
	protected Text incorrectGuesses;
	protected Text incorrectLetters;
    ...
    public void keyTyped(KeyEvent e)
    {
    	if(setup) {
    		...
    		if (!flag) {
    			...
    			incorrectCount++;
    			if (incorrectCount < 6) {
    				//Add comma if not last letter
    				incorrectText += String.valueOf(guessedLetter) + ", ";
    			} else {
    				//If last letter, omit comma
    				incorrectText += String.valueOf(guessedLetter);
    			}
    			
    			incorrectGuesses.show();
    		...
    		if (gallows.isAlive()) {
    			...
    			label.setText("Game over! Player 2 wins.");
    ...
    public void onMousePress(Location point) {
    	if (button.contains(point) && !button.isHidden() && ! word.isEmpty()) {
            ...
            //Remove incorrectGuesses from last round
            //Initialize Text incorrectGuesses (label) and hide it
            incorrectGuesses = new Text("Incorrect guesses: ", WINDOW_SIZE * 7/11.0, WINDOW_SIZE * 3/11, canvas);
            incorrectGuesses.setFontSize(20);
            incorrectGuesses.hide();
            incorrectText = "";
            //Initialize Text incorrectLetters (letters) and hide it
            incorrectLetters = new Text("", WINDOW_SIZE * 7/11.0, WINDOW_SIZE * 3.5/11, canvas);
            incorrectLetters.setFontSize(20);
    ...
    public static void main(String[] args) { 
        new Extension1().startController(WINDOW_SIZE, WINDOW_SIZE); 
	}
